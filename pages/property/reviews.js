...
function Reviews() {
  ...
  const delConfirm =(id)=>{
    ...
    var url=`/api/${del}`;
    alert(url);
    axios.delete(`${url}`).then(()=>{fetchReviews(); alert(`${del} deleted sucessfully`); setModelDel(0)}).catch(()=>alert("some error occured"))
  }
    ...
    <div className="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 backdrop-blur-xl bg-black/30 md:inset-0 z-50 flex justify-center items-center h-modal sm:h-full">
            <div className="relative w-full max-w-2xl px-4 h-full md:h-auto">
              <div className="bg-white rounded-lg shadow relative m-4 px-4 py-6">
                <span className="mx-4 capitalize bold">Are you sure you want to delete this review?</span><br/>
                <button onClick={()=>delConfirm()} className='m-2 bg-gradient-to-r bg-green-600 hover:bg-green-700 text-white  sm:inline-flex 
                                                           font-semibold rounded-lg text-sm px-5 py-2 text-center items-center ease-linear transition-all
                                                            duration-150'>Yes</button>
                <button onClick={()=>setModelDel(0)} className='ml-4 mt-2 bg-gradient-to-r bg-red-600 hover:bg-red-700 text-white  sm:inline-flex 
                                                           font-semibold rounded-lg text-sm px-5 py-2 text-center items-center ease-linear transition-all
                                                            duration-150'>No</button>
