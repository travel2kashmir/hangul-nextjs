   ...
   // Inventory
    ...
    if(data?.inventory_min !== "" && data?.inventory_min !== undefined ){
       if(data?.inventory_max === "" || data?.inventory_max === undefined) {
        flag.push(false);
        error.inventory_max="The occupancy maximum is required."
       }
   }
   if(data?.inventory_max !== "" && data?.inventory_max !== undefined ){
   if (data?.inventory_min === "" || data?.inventory_min ===undefined){
    ...
    flag.push(false);
    error.inventory_min="The occupancy minimum is required."
   }    
  }
   ...
   if(data?.inventory_min >= data?.inventory_max)
  {
     ...
     error.inventory_max="The inventory max should be greater than the inventory min."
       ...
       if(!(/^([1-9]+[0-9]*)$/.test(data.inventory_min))){
           flag.push(false);
           error.inventory_min="The inventory min accepts only numbers."
          } 
          if(!(/^([1-9]+[0-9]*)$/.test(data.inventory_max))){
           flag.push(false);
           error.inventory_max="The inventory maximum accepts only numbers."
          }
   }
  }
