...
function Modifications() {
    ...
    useEffect(() => {
     ...
     const fetchModifications = async () => {
       try {
         var genData=[];
           const url = `/api/ari/property_rate_modifications/${currentProperty.property_id}`
           const response = await axios.get(url, { headers: { 'accept': 'application/json' } });
          setAllModifications(response.data)
        
         response?.data?.map((item)=>{
           var temp={
             name:item.modification_name,
             status:item.status,
             id:item.modification_id
           } 
           genData.push(temp)
         }
         )
 ...
 const submitModificationEdit = (props) => { 
  localStorage.setItem("modificationId", props?.id);
  Router.push("./modifications/modification")
};
// Add Promotion
...
const submitModificationDelete = (props) => {
 ...
 const url = `/api/ari/property_rate_modifications/${props}`;
  axios
     .delete(url)
     .then((response) => {
            ...
            <ol className="inline-flex items-center space-x-1 md:space-x-2">
              ...
              <li>
                ...
                <div className={`${color?.textgray} text-base font-medium  inline-flex items-center`}>
                  <svg className="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd"></path></svg>
                  <span className="text-gray-400 ml-1 md:ml-2 font-medium text-sm  " aria-current="page">{language?.ratemodifications}</span>
                </div>
                </div>
              </li>
            </ol>
...
Modifications.getLayout = function PageLayout(page){
    return(
      <>
      {page}
      </>
